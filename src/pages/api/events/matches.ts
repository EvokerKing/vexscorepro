...
export async function GET(params: any) {
	...
	*/
	...
	const matches = []
	const res = await fetch(`https://www.robotevents.com/api/v2/events/${id}/divisions/${div}/matches?per_page=250&${searchParams}`, {
		headers: {
			"accept": "application/json",
			"Authorization": `Bearer ${key}`
		}
	...
	if (body.meta.from != body.meta.last_page) {
		for (let i = 2; i <= body.meta.last_page; i++) {
			const iRes = await fetch(`https://www.robotevents.com/api/v2/events/${id}/divisions/${div}/matches?per_page=250&page=${i}&${searchParams}`, {
				headers: {
					"accept": "application/json",
					"Authorization": `Bearer ${key}`
				}
